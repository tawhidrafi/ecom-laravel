        ...
        <form method="POST" enctype="multipart/form-data" action="{{ route('products.update', $product) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    @if($product->image)
                        <img id="mainImagePreview" src="{{ asset('assets/upload/product/' . $product->image) }}"
                    ...
                    <div class="flex flex-wrap gap-2 mb-2">
                        @if(!empty($product->images))
                            @foreach($product->images as $img)
                                <img src="{{ asset('uploads/products/gallery/' . $img) }}" class="w-24 h-24 object-cover rounded">
                            @endforeach
                        @endif
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="regular_price" value="{{ old('regular_price', $product->price) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="quantity" value="{{ old('quantity', $product->stock) }}"
                        class="w-full border px-3 py-2 rounded">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="stock_status" class="w-full border px-3 py-2 rounded">
                        <option value="instock" {{ $product->in_stock == 'instock' ? 'selected' : '' }}>In Stock</option>
                        <option value="outofstock" {{ $product->in_stock == 'outofstock' ? 'selected' : '' }}>Out of Stock
                        </option>
                    </select>
                </div>
                <div>
            ...
            <button type="submit" class="w-full bg-gray-700 text-white px-4 py-2 rounded hover:bg-gray-800">Update
                Product</button>
        </form>
    </div>
@endsection
